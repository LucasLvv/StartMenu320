...
//根据键值得到point类型数据
CCPoint StaticData::pointFromKey(const std::string& key) 
{
	return CCPointFromString(_dictionary->valueForKey(key.c_str())->getCString());
}

//根据键值得到rect类型数据
CCRect StaticData::rectFromKey(const std::string& key) 
{
	return CCRectFromString(_dictionary->valueForKey(key.c_str())->getCString());
}

CCSize StaticData::sizeFromKey(const std::string& key) 
{
